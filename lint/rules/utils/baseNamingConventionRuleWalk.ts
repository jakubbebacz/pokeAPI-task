...
export abstract class BaseNamingConventionRuleWalk {
    ...
    protected memberIsPrivate(node: ts.TypeParameterDeclaration): boolean {
        return hasModifier(node.modifiers, ts.SyntaxKind.PrivateKeyword);
    }

    protected memberIsStatic(node: ts.TypeParameterDeclaration): boolean {
        return hasModifier(node.modifiers, ts.SyntaxKind.StaticKeyword);
    }

    protected memberIsReadonly(node: ts.TypeParameterDeclaration): boolean {
